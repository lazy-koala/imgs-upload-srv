<template>
<div class="">
  <div v-show="data.status == '00'" class="imgage-wrapper" v-loading="loading" @mouseover="toggleShow(data._id, 1)" @mouseleave="toggleShow(data._id, 0)">
    <el-card :body-style="{ padding: '0px' }">
      <div style="position: relative">
        <el-image
          class="image"
          :src="data.thumbUrl || data.url"
          ::preview-src-list="[data.thumbUrl || data.url]"
          @load="hideLoading()"
          fit="cover"></el-image>
      ...
      <div style="padding: 5px; font-size: 12px">
        ...
        <p style="color: rgba(0, 0, 0, 0.65); padding-top: 5px;">{{dateFormat(data.createTime)}}</p>
        <!-- <div class="time">{{dateFormat(data.createTime)}}</div> -->
      </div>
    </el-card>
  ...
  <!-- 删除图片弹框 -->
  <el-dialog title="提示" :visible.sync="showDel" width="40%" center>
    <span>是否确认删除该图片?</span>
  ...
  <!-- 编辑图片开始 -->
  <el-dialog :visible.sync="showZoomIn" v-if="showZoomIn" width="45%" height="50%" title="图片编辑" center>
    <div class="zoomin-wrapper" v-loading="editLoading">
      ...
      <div class="sort">
        <el-select v-model="selectedSort" filterable placeholder="请选择分类">
          <el-option v-for="item in sortList" :key="item.sortId" :label="item.sortName" :value="item.sortId">
          </el-option>
        </el-select>
      </div>
      <div class="tag-list" v-if="tagList">
        ...
        <el-input v-if="showTagInput" v-model="inputValue" ref="saveTagInput" size="small" class="input-new-tag" maxlength=6 minlength=3 @keyup.enter.native="handleInputConfirm" @blur="handleInputConfirm">
        </el-input>
...
export default {
  ...
  methods: {
    ...
    deleteImg: function (id) {
      ...
      var that = this;
      $axios.post('/api/imgs/del', {
        ids: [id ? id : that.delId]
      }).then((res) => {
        if (res.data) {
          Message.success({
            message: '删除成功',
    ...
    recover(id) {
      $axios.post('/api/imgs/confirm_to_visit', {
        imgId: id
      }).then((res) => {
        if (res.data) {
          Message.success({
